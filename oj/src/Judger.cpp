...
bool Judger::Init(Json &initjson)
{
    // 初始化数据
    // Json(SubmitId,ProblemId,JudgeNum,Code,Language,TimeLimit,MemoryLimit)
    LOG_DEBUG << initjson.dump();
    m_submitid = initjson["SubmitId"].get<std::string>();
    m_problemid = initjson["ProblemId"].get<std::string>();
    m_code = initjson["Code"].get<std::string>();

    m_judgenum = initjson["JudgeNum"].get<int>();
    m_timelimit = initjson["TimeLimit"].get<int>();
    m_memorylimit = initjson["MemoryLimit"].get<int64_t>() * 1024 * 1024;
    m_language = initjson["Language"].get<std::string>();
    ...
    RUN_PATH = "/home/thebs/mymuduo/oj/datadir/run/" + m_submitid + "/";
    DATA_PATH = "/home/thebs/mymuduo/oj/datadir/data/" + m_problemid + "/";

    m_resjson.clear();

    // 创建中间文件夹
    m_command = "mkdir " + RUN_PATH;
...
bool Judger::RunProgramC_Cpp()
{
    ...
    execConfig.realTimeLimit = m_maxtimelimit;
    execConfig.cpuTimeLimit = m_timelimit;
    execConfig.memoryLimit = m_maxmemorylimie;
    execConfig.outputLimit = 10 * 1024 * 1024;
    string errPath = RUN_PATH + "error.out";
    execConfig.stderrPath = (char *)errPath.data();
    string exe_path = RUN_PATH + "main";
    execConfig.execPath = (char *)exe_path.data();
    string log_path = RUN_PATH + "judger.log";
    execConfig.loggerPath = (char *)log_path.data();
    execConfig.loggerFile = fopen(execConfig.loggerPath, "w");
    execConfig.guard = 1;

    RunProgram(&execConfig);
    return true;
}
...
bool Judger::RunProgram(execConfig* conf) {
    ...
    for (int i = 1; i <= m_judgenum; i++)
    {
        string index = to_string(i);
        string input_path = DATA_PATH + index + ".in";
        conf->stdinPath = (char *)input_path.data();
        string output_path = RUN_PATH + index + ".out";
        conf->stdoutPath = (char *)output_path.data();
        runJudge(conf, &judgeResult);
        // 执行程序
        // run(conf, &res);
        printf("cpu time is %lu,real time is %lu,memory is %ld,signal is %d\n", judgeResult.cpuTimeCost, judgeResult.realTimeCost, judgeResult.memoryCost, judgeResult.condition);

        JudgmentResult(&judgeResult, index);
    }
    return true;
}

bool Judger::JudgmentResult(struct judgeResult *res, string &index)
{
    Json testinfo; // 保存本次结果
    // 获取最大时间和空间
    m_runtime = max((rlim_t)m_runtime, res->cpuTimeCost);
    m_runmemory = max((rlim_t)m_runmemory, res->memoryCost);

    // 获取运行时间和运行内存的数据
    testinfo["RunTime"] = to_string(res->cpuTimeCost) + "MS";
    testinfo["RunMemory"] = to_string(res->memoryCost / 1024 / 1024) + "MB";
    ...
    // 判断结果
    if (res->condition == RUNNING_CONDITION::RUN_SUCCESS)
    {
        // 判断是否超出时间限制
        if (res->cpuTimeCost > m_timelimit)
        {
            m_result = TLE;
            testinfo["Status"] = TLE;
        }
        else if (res->memoryCost > m_memorylimit)
        {
            m_result = MLE;
            testinfo["Status"] = MLE;
        }
    ...
    else if (res->condition == RUNNING_CONDITION::TIME_LIMIT_EXCEED) // CPU_TIME_LIMIT_EXCEEDED
    {
        m_result = TLE;
        testinfo["Status"] = TLE;
    }
    else if (res->condition == RUNNING_CONDITION::TIME_LIMIT_EXCEED) // REAL_TIME_LIMIT_EXCEEDED
    {
        m_result = TLE;
        testinfo["Status"] = TLE;
    }
    else if (res->condition == RUNNING_CONDITION::MEMORY_LIMIT_EXCEED) // MEMORY_LIMIT_EXCEEDED
    {
        m_result = MLE;
        testinfo["Status"] = MLE;
    }
    else if (res->condition == RUNNING_CONDITION::RUNTIME_ERROR)
    {
        // 获取失败原因
        ifstream infile;
    ...
    else if (res->condition == RUNNING_CONDITION::SEGMENTATION_FAULT)
    {
        // 获取失败原因
        ifstream infile;
    ...
    m_resjson["TestInfo"].push_back(testinfo);
    return testinfo["Status"] == AC;
}
